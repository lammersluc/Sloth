    ...
    run: async (client, message, args) => {
        ...
        message.channel.send({ embeds: [embed
            ...
            message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] }).then(collected => {
                ...
                msg.edit({ embeds: [embed2.setDescription(`Jumped to song position \`${position}\` in the queue.`)] });

            });
            
        }).catch(() => message.channel.send({ embeds: [embed2.setDescription('You didn\'t choose anything after 30 seconds.') ]}) );
